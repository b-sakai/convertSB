        ...
        with open(filename, 'r') as f:
            for line in f:
                if line[0] == '#':
                    ...
                    if one_data['parent'] != -1:
                        self.L[int(record[0])] = self.calcL(self.data[one_data['parent']]['pos'], one_data['pos'])
                    else:
                        self.L[int(record[0])] = 1.0

    def calcL(self, posParent, posCurrent):
        dx = posParent[0] - posCurrent[0]
        dy = posParent[1] - posCurrent[1]
        dz = posParent[2] - posCurrent[2]
